...
describe('It must handle basic inputs', function() {
  ...
  describe('It must handle formulae with references', function() {
    ...
    dataObj.storageArr[0][5][0] = '=5'; /** A6 */
    describe('It must handle formula with simple references', function() {
      it(`Must resolve a reference to an empty cell [=b5] '<empty>'`, function() {
        expect(parseExpression(`=b5`, dataObj)).to.equal(`0`);
      });
      it(`Must resolve a reference to a cell with a number [=a2] '=1'`, function() {
        expect(parseExpression(`=a2`, dataObj)).to.equal(`1`);
      });
    });
    describe('It must handle formulae with references and numbers', function() {
      it(`Must resolve a reference to a cell with a simple formula [=a3] '=a2*2'`, function() {
        expect(parseExpression(`=a3`, dataObj)).to.equal(`2`);
      });
      it(`Must resolve a reference to a cell with a simple formula containing whitespaces [= a4] '=A2 + a3'`, function() {
  ...
  describe('It must handle formulae with built-in functions', function() {
    describe('It must sum a range numbers and place the result in a cell', function() {
      dataObj.storageArr[0][6][0] = '=SUM(A2:a4)'; /** A7 */
      it(`Must retrieve a cell containg a simple sum [=A7] '=SUM(A2:a4)'`, function() {
        expect(parseExpression(`=A7`, dataObj)).to.equal(`6`);
      });
    });
    describe('It must perform a complex combination of operations and SUM functions over a range', function() {
      it(`Must add a reference to a SUM() of listed refs containing a SUM() of a range [=A1 + A2 + SUM(A3, a4, SUM(A5:A7)]`, function() {
        expect(parseExpression(`=A1 + A2 + SUM(A3, a4, SUM(A5:A7))`, dataObj)).to.equal(`21`);
      });
    });
  });
});
